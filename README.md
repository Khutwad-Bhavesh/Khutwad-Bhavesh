#  Bhavesh Khutwad's Portfolio

A collection of projects I've built while learning and experimenting with different technologies.

## Projects

| Project | Tech | Description | Repo |
|---------|------|-------------|------|
|  Sorting Visualizer | Java, Swing | Animated sorting algorithm visualizer with neon UI, 6 algorithms, live stats | [View](link) |
|  Crown Clash | Java | A turn-based strategy game | [View](link) |
|  File Converter | Python | Convert files between different formats | [View](link) |
|  App UI | UI Design | Mobile app interface design | [View](link) |

## Skills
**Languages:**
Java, Python, JavaScript, HTML, CSS

**Libraries & Frameworks:**
Java Swing, Node.js

**Tools & Platforms:**
Git, GitHub, VS Code, Android Studio, Figma, Postman, Godot

**Concepts:**
Object Oriented Programming, Data Structures & Algorithms,
Computer Networks, Operating Systems, Database Management

##  Contact
Open to internship opportunities — feel free to reach out!
